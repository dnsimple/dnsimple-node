...
class Zones {
   ...
   * client.zones.allRecords(1010, 'example.com').then(function(items) {
   *   // use items
   * }, function(error) {
   *   // handle error
   * });
   ...
   * client.zones.allRecords(1010, 'example.com', {sort: 'name:asc'}).then(function(items) {
   *   // use items
   * }, function(error) {
   *   // handle error
   * });
   ...
   * @return {Promise}
   */
  allRecords(accountId, zoneId, options = {}) {
    return new Paginate(this).paginate(this.listRecords, [accountId, zoneId, options]);
  }
