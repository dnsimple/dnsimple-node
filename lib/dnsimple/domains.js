...
class Domains {
   ...
   *
   * This method is similar to {#domains}, but instead of returning the results of a
   ...
   * client.domains.allDomains(1010).then(function(domains) {
   *   // use domains list
   * }, function(error) {
   *   // handle error
   * });
   *
   * @example List domains, provide a sorting policy
   * client.domains.allDomains(1010, {sort: 'name:asc'}).then(function(domains) {
   *   // use domains list
   * }, function(error) {
   *   // handle error
   * });
   *
   * @example List domains, provide a filtering policy
   * client.domains.allDomains(1010, {name_like: 'example'}).then(function(domains) {
   *   // handle response
   * }, function(error) {
   *   // handle error
   * });
  ...
  allDomains(accountId, options = {}) {
    var self = this;
    var listFunction = this.listDomains;
    var args = [accountId, options];

    return new Promise(function(resolve, reject) {
      self._client._nextPage.call(self, listFunction, args).then(function(responses) {
        let domains = [].concat.apply([], responses.map(function(response) { return response.data; }));
        resolve(domains);
      });
    });
  }
