...
const dnsimple = require("../lib/dnsimple")(testUtils.getAccessToken());
...
describe("dnsimple module", () => {
  ...
  describe("#setUserAgent", () => {
    it("respects the default User-Agent", () => {
      expect(dnsimple._api.userAgent).to.equal(
        "dnsimple-node/" + dnsimple.VERSION
      );
    });
    ...
    it("composes the User-Agent", () => {
      ...
      expect(dnsimple._api.userAgent).to.equal(
        "my-app dnsimple-node/" + dnsimple.VERSION
      );
    });
  });
});
