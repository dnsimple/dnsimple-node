...
describe("response handling", () => {
  describe("a 400 error", () => {
    it("includes the error message from the server", async () => {
      nock("https://api.dnsimple.com").post("/v2/validation-error", {}).reply(readFixtureAt("validation-error.http"));
  ...
  describe("a 200 response with malformed json", () => {
    it("produces a JSON parse error", async () => {
      nock("https://api.dnsimple.com").get("/v2/success-with-malformed-json").reply(readFixtureAt("success-with-malformed-json.http"));
  ...
  describe("an error response with HTML content", () => {
    it("produces a JSON parse error", async () => {
      nock("https://api.dnsimple.com").get("/v2/badgateway").reply(readFixtureAt("badgateway.http"));
  ...
  describe("a 405 error", () => {
    it("results in a rejected promise", async () => {
      nock("https://api.dnsimple.com").get("/v2/method-not-allowed").reply(readFixtureAt("method-not-allowed.http"));
